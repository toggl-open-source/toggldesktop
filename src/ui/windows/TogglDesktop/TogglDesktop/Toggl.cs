...
namespace TogglDesktop
{
    public static partial class Toggl
    {
        ...
        public static bool DiscardTimeAt(string guid, long at, bool split)
        {
            return toggl_discard_time_at(ctx, guid, at, split);
        }

        public static bool DiscardTimeAndContinue(string guid, long at, bool split)
        {
        ...
        {
            if (!toggl_set_settings_use_idle_detection(ctx, settings.UseIdleDetection))
            {
                return false;
            }

            if (!toggl_set_settings_on_top(ctx, settings.OnTop))
            {
                return false;
            }

            if (!toggl_set_settings_reminder(ctx, settings.Reminder))
            {
                return false;
            }

            if (!toggl_set_settings_idle_minutes(ctx, (ulong)settings.IdleMinutes))
            {
                return false;
            }

            if (!toggl_set_settings_reminder_minutes(ctx, (ulong)settings.ReminderMinutes))
            {
                return false;
            }

            if (!toggl_set_settings_focus_on_shortcut(ctx, settings.FocusOnShortcut))
            {
                return false;
            }

            if (!toggl_set_settings_autodetect_proxy(ctx, settings.AutodetectProxy))
            {
                return false;
            }
        ...
        public static void SetIdleSeconds(ulong idle_seconds)
        {
            toggl_set_idle_seconds(ctx, idle_seconds);
        }

        public static string FormatDurationInSecondsHHMMSS(long duration_in_seconds)
        {
        ...
        public static string Env { get => env; set => env = value; }
        ...
        private static void DeleteOldUpdates()
        {
            ...
            foreach (var file in di.GetFiles("TogglDesktopInstaller*.exe", SearchOption.TopDirectoryOnly))
            {
                ...
                catch (Exception e)
                {
                    ...
                    OnError?.Invoke($"Unable to delete the file: {file.FullName}. Delete this file manually.", false);
                }
            }
        ...
        public static DateTime DateTimeFromUnix(ulong unix_seconds)
        {
            return UnixEpoch.AddSeconds(unix_seconds).ToLocalTime().DateTime;
        }

        public static long UnixFromDateTime(DateTime value)
        {
