...
public static class Utils
{
    ...
    public static SolidColorBrush AdaptedProjectColorBrushFromString(string hex)
    {
        ...
        private const string PoliciesRegistryPath = @"Software\Policies\Toggl\TogglTrack";
        public static bool GetLaunchOnStartupRegistry()
        {
            var subKey = Registry.CurrentUser.OpenSubKey(StartupAppsRegistryPath);
            return subKey?.GetValue("TogglTrack") != null;
        }
        ...
        public static void SaveLaunchOnStartupRegistry(bool launchOnStartup)
        {
                         ...
                         ?? Registry.CurrentUser.CreateSubKey(StartupAppsRegistryPath))
            {
                ...
                if (launchOnStartup)
                {
                    subKey.SetValue("TogglTrack", "\"" + Assembly.GetEntryAssembly().Location + "\"" + " --minimize");
                }
                else
                {
                    subKey.DeleteValue("TogglTrack");
                }
            }
        }
