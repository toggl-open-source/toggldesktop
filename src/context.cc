...
#include "./timeline/timeline_uploader.h"
#include "./timeline/window_change_recorder.h"
...
, next_wake_at_(0)
, gui_update_(new GUIUpdate(this))
...
bool Context::isTimeEntryLocked(TimeEntry* te) {
    return isTimeLockedInWorkspace(te->Start(),
                                   user()->related.WorkspaceByID(te->WID()));
}
...
bool Context::canChangeProjectTo(TimeEntry* te, Project* p) {
    return !isTimeLockedInWorkspace(te->Start(),
                                    user()->related.WorkspaceByID(p->WID()));
}
