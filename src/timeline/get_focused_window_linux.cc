...
void GetFocusedWindowInfo(std::string *title, std::string *filename) {
  ...
  *filename = "";
  ...
  char *prop = get_property(display,
    DefaultRootWindow(display), XA_WINDOW, kNetActiveWindow, &size);
  if (prop) {
  ...
  if (active_window) {
    char *net_wm_name = get_property(display, active_window,
