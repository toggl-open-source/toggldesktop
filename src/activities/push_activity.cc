...
PushActivity::PushActivity(ActivityManager *parent)
    : toggl::Activity(parent)
{

}

void PushActivity::work() {
    syncerActivity();
}

void PushActivity::syncerActivity() {
    while (true) {
...

        {
            ...
            if (trigger_sync_) {
                TogglClient client(context()->UI());

                error err = pullAllUserData(&client);
                if (err != noError) {
                    context()->displayError(err);
                }

                context()->setOnline("Data pulled");
                ...
                context()->displayError(context()->save(false));
            }
...
void PushActivity::push() {

    trigger_push_ = true;
    activity_.start();
}

void PushActivity::sync() {

    trigger_sync_ = true;
    activity_.start();
}

error PushActivity::pullAllUserData(TogglClient *toggl_client) {
    ...
    try {
        ...
        context()->UserVisit([&](User *user_){
            ...
            if (running_entry && !new_running_entry) {
                context()->resetLastTrackingReminderTime();
            }
        });
        if (err != noError)
            return err;
    ...
    try {
        ...
        *had_something_to_push = true;
        ...
        if (projects.size() > 0) {
            ...
            err = context()->updateEntryProjects(
                        projects,
                        time_entries);
            if (err != noError) {
                return err;
...
error PushActivity::pushEntries(std::map<std::string, BaseModel *> models, std::vector<TimeEntry *> time_entries, std::string api_token, TogglClient toggl_client) {
...
error PushActivity::pullWorkspaces(TogglClient *toggl_client) {
    std::string api_token;
    context()->UserVisit([&](User *user_){
        api_token = user_->APIToken();
    });
...
error PushActivity::pullWorkspacePreferences(TogglClient *toggl_client) {
...
error PushActivity::pullWorkspacePreferences(TogglClient *toggl_client, Workspace *workspace, std::string *json) {
    std::string api_token;
    context()->UserVisit([&](User *user_){
        api_token = user_->APIToken();
    });
...
error PushActivity::pullUserPreferences(TogglClient *toggl_client) {
    std::string api_token;
    context()->UserVisit([&](User *user_) {
        api_token = user_->APIToken();
    });
...
template<typename T>
void PushActivity::collectPushableModels(const std::set<T *> &list, std::vector<T *> *result, std::map<std::string, BaseModel *> *models) {
