...
void GUIUpdate::update(const UIElements &what, UserData *user) {
    ...
    */
    ...
    // Collect data
    {
        ...
        if (what.display_time_entry_autocomplete) {
            if (what.first_load) {
                if (user) {
                    user->TimeEntryAutocompleteItems(&time_entry_autocompletes);
                }
        ...
        if (what.display_mini_timer_autocomplete) {
            if (what.first_load) {
                if (user) {
                    user->MinitimerAutocompleteItems(&minitimer_autocompletes);
                }
        ...
        if (what.display_client_select)
            renderClientSelect(user);

        if (what.display_timer_state)
            renderTimerState(user);
        ...
        if (what.display_project_autocomplete) {
            if (what.first_load) {
                if (user) {
                    user->ProjectAutocompleteItems(&project_autocompletes);
                }
        ...
        if (what.display_unsynced_items)
            renderUnsyncedItems(user);
    }
}

void GUIUpdate::renderTimeEntryEditor(UserData *user, const UIElements &what) {
    view::TimeEntry editor_time_entry_view;
    std::vector<view::Generic> tag_views;

    auto editor_time_entry = user->TimeEntries.findByGUID(what.time_entry_editor_guid);
    if (editor_time_entry) {
        ...
        if (editor_time_entry->WID()) {
            ws = user->Workspaces.findByID(editor_time_entry->WID());
        }
        if (ws) {
        ...
        editor_time_entry_view.CanSeeBillable = user->CanSeeBillable(ws);
        editor_time_entry_view.DefaultWID = user->DefaultWID();

        editor_time_entry_view.Locked = user->isTimeEntryLocked(editor_time_entry);

        // Display tags also when time entry is being edited,
        // because tags are filtered by TE WID
        std::vector<std::string> tags;
        user->TagList(&tags, editor_time_entry->WID());
...
void GUIUpdate::renderWorkspaceSelect(UserData *user) {
...
void GUIUpdate::renderTimerState(UserData *user) {
    ...
    if (running_entry) {
        ...
        running_entry_view.Duration =
            toggl::Formatter::FormatDuration(
                ...
                Format::Classic);
        running_entry_view.DateDuration = Formatter::FormatDurationForDateHeader(user->TotalDurationForDate(running_entry));
        user->ProjectLabelAndColorCode(running_entry, &running_entry_view);
    }
...
void GUIUpdate::renderTimeEntries(UserData *user, bool open_time_entry_list) {
    ...
    std::vector<TimeEntry *> time_entries = user->VisibleTimeEntries();
    ...
    for (unsigned int i = 0; i < time_entries.size(); i++) {
                ...
                if (group_header_id[view.GroupName] == i) {
                    ...
                    if (entry_groups[view.GroupName]) {
                        for (unsigned int j = 0; j < group_items[view.GroupName].size(); j++) {
                            ...
                            user->ProjectLabelAndColorCode(group_entry, &group_entry_view);

                            group_entry_view.Locked = user->isTimeEntryLocked(group_entry);
                    ...
                    group_view.Fill(te);
                    user->ProjectLabelAndColorCode(te, &group_view);
        ...
        user->ProjectLabelAndColorCode(te, &view);
    }
    // RENDERING PART //
...
void GUIUpdate::renderUnsyncedItems(UserData *user) {
    Poco::Int64 unsynced_item_count = user->NumberOfUnsyncedTimeEntries();
    UI()->DisplayUnsyncedItems(unsynced_item_count);
}

void GUIUpdate::renderAutotrackerRules(UserData *user) {
    ...
    if (UI()->CanDisplayAutotrackerRules()) {
        // Collect rules
        auto autotrackerRules = user->AutotrackerRules();
        for (auto it = autotrackerRules->begin(); it != autotrackerRules->end(); ++it) {
            AutotrackerRule *model = *it;
            auto p = user->Projects.findByID(model->PID());
            auto t = user->Tasks.findByID(model->TID());
...
void GUIUpdate::OpenTimeEntryList(UserData *user) {
...
void GUIUpdate::ResetAutotrackerView(UserData *user) {
