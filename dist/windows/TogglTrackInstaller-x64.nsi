...
!else
  ...
  OutFile "TogglTrackInstaller-x64.exe"
...
;General
  ...
  Icon "..\..\src\ui\windows\TogglTrack\TogglTrack\Resources\toggl.ico"

  ;Default installation folder. Local app data does not need
  ;admin privileges to install.
  InstallDir "$LOCALAPPDATA\TogglTrack"

  ;Get installation folder from registry if available
  InstallDirRegKey HKCU "Software\TogglTrack" ""
...
;Icons
  ...
  !define MUI_ICON "..\..\src\ui\windows\TogglTrack\TogglTrack\Resources\toggl.ico"
...
;Run App after install
  ...
  !define MUI_FINISHPAGE_RUN "$INSTDIR\TogglTrack.exe"
...
!define srcdir "..\..\src\ui\windows\TogglTrack\TogglTrack\bin\x64\Release"
...
Section
  ...
  ${If} $isNewUpdater == 0
    ...
    DetailPrint "Closing all old TogglTrack processes"
    File "NSIS_plugins\KillProc.exe"
    nsExec::Exec "$INSTDIR\KillProc.exe TogglTrack"
  ...
  File "${srcdir}\TogglTrack.exe.config"
  File "..\..\src\ui\windows\TogglTrack\TogglTrack\Resources\toggl.ico"

  ${If} $isOldUpdater == 0
  ${AndIf} $isNewUpdater == 0    
    ;Store installation folder
    WriteRegStr HKCU "Software\TogglTrack" "" $INSTDIR
  ${EndIf}
...
!endif
  ...
  ${If} $isOldUpdater == 0
  ${AndIf} $isNewUpdater == 0
    CreateShortCut "$DESKTOP\TogglTrack.lnk" "$INSTDIR\TogglTrack.exe" ""
  ${Else}
    ; Overwrite the possibly faulty shortcut (#4005)
    IfFileExists $DESKTOP\TogglTrack.lnk 0 +3
      SetOutPath "$LOCALAPPDATA\TogglTrack"
      CreateShortCut "$DESKTOP\TogglTrack.lnk" "$LOCALAPPDATA\TogglTrack\TogglTrack.exe" ""
  ${EndIf}

  ${If} $isOldUpdater == 0
  ${AndIf} $isNewUpdater == 0
    ;Add/Remove programs entry
    !define REG_UNINSTALL "Software\Microsoft\Windows\CurrentVersion\Uninstall\TogglTrack"
    WriteRegStr HKCU "${REG_UNINSTALL}" "DisplayName" "Toggl Track"
    WriteRegStr HKCU "${REG_UNINSTALL}" "DisplayIcon" "$\"$INSTDIR\TogglTrack.exe$\""
    ...
    createShortCut "$SMPROGRAMS\Toggl\Toggl Track.lnk" "$INSTDIR\TogglTrack.exe" "" "$INSTDIR\toggl.ico"
...
Section "Uninstall"
  ...
  Delete "$INSTDIR\*.exe"
  Delete "$INSTDIR\TogglTrack.exe.config"
  Delete "$INSTDIR\toggl.ico"
  RMDir "$INSTDIR\updates"
  RMDir /r "$LOCALAPPDATA\Onova\TogglTrack" ;Remove the prepared updates

  ;Delete desktop shortcut
  Delete "$DESKTOP\TogglTrack.lnk"

  RMDir "$INSTDIR"

  DeleteRegKey /ifempty HKCU "Software\TogglTrack"

  ;Remove uninstall info from Control Panel
  DeleteRegKey HKCU "Software\Microsoft\Windows\CurrentVersion\Uninstall\TogglTrack"

  ; Remove run on Windows login entry
  DeleteRegValue HKCU "Software\Microsoft\Windows\CurrentVersion\Run" "TogglTrack"
...
Function .onInstSuccess
  ...
  ${if} $isOldUpdater == 1
    Exec "$INSTDIR\TogglTrack.exe --updated"
  ${Endif}

FunctionEnd

Function runOnStartup
  WriteRegStr HKCU "Software\Microsoft\Windows\CurrentVersion\Run" "TogglTrack" '"$INSTDIR\TogglTrack.exe" --minimize'
FunctionEnd
...
Function un.customPage
  !insertmacro MUI_HEADER_TEXT "Uninstall Toggl Track" "Remove Toggl Track from your computer."
  StrCpy $deleteData 0
  nsDialogs::Create 1018
  Pop $0

  ${NSD_CreateLabel} 0 0 100% 40u "Toggl Track will be uninstalled from the following folder. Click Uninstall to start the uninstallation."
...
Function un.onUninstSuccess
  ...
  ${If} $deleteData == 1
    Delete "$INSTDIR\toggltrack.db"
    Delete "$INSTDIR\toggltrack.log"
    RMDir /r $INSTDIR
  ${EndIf} 
